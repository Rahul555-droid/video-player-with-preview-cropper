...
import { ASPECT_RATIOS , defaultCropperState , DEFAULT_ASPECT_RATIO } from 'VideoPlayer/constant'

export const useCropper = (containerRef, videoRef) => {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    const newX = Math.max(
      0,
      Math.min(mouseX - cropper.width / 2, containerRef.current.clientWidth - cropper.width)
    )
